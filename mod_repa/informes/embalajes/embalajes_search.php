<?php
    ...
    $fechaDesde             = new DateTime($_POST['desdeFechaEmbalaje'].' '.'00:00:00');
    $formatArgDesde         = $fechaDesde->format('Y-m-d H:i:s');
    $fechaHasta             = new DateTime($_POST['hastaFechaEmbalaje'].' '.'23:59:59');
    $formatArgHasta         = $fechaHasta->format('Y-m-d H:i:s');
    $embalador              = filter_var($_POST['embalador'], FILTER_SANITIZE_STRING);
    ...
    $query = "  SELECT 
                ...
                WHERE
                    rep3_reparaciones.fembalaje             BETWEEN '{$formatArgDesde}' AND '{$formatArgHasta}'
                AND
                    rep3_reparaciones.anulado               <> 'S'
                AND
                    rep3_reparaciones.estado_id             IN ('6','7')
                
            "; 
    if(!empty($embalador)){
        $query .= " AND rep3_reparaciones.embalador_id      = '{$embalador}'"; 
    }  
