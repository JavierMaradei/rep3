(() =>{
    ...
    let tabla = $('#tabla').DataTable( {
        ...
        buttons: [
            {extend: 'copy'},
            {extend: 'csv'},
            {extend: 'excel', title: 'Informe de ingresos'},
            {extend: 'pdf', title: 'Informe de ingresos'},
            {extend: 'print',}
        ],
        "bLengthChange": false,
        "iDisplayLength": 10,
    ...
    function recargaTabla(){
        ...
        xhr.open('POST', 'mod_repa/informes/ingresos/ingresos_search.php')//Envío la información del filtro
        xhr.send(formData)
    ...
    btnBuscarProducto.addEventListener('click', e => {
        ...
        busquedaPorProducto('#modal', '#body', '#titulo').then(() => {
