(function iniciarPerfiles(){
    ...
    let activoPerfiles      = document.querySelector('#activoPerfiles')//Captura de boton cancelar
    let edit                = false//flag de edición de registro existente o nuevo registro
    let id                  = ''
    let arrayVal = {
        idPerfiles          : {readonly : true},
        descripcionPerfiles : {required: true, maxlength: 50, validated: true},
        activoPerfiles      : {}
    }
    ...
    let tabla = $('#tabla_perfiles').DataTable( {
        "ajax": {
            url: 'mod_repa/configuracion/perfiles/perfiles_list.php',
            type: 'GET',
            dataSrc: ""
        },
        "columns": [  
    ...
    $(document).on('click', '.task-item', (e) => {
        ...
        url = 'mod_repa/configuracion/perfiles/perfiles_single.php'
        showData(id, url, inputs)
        $(btnEliminaPerfiles).show()
        edit = true
    })

    //Funcionalidad del botón de Grabar
    btnGrabaPerfiles.addEventListener('click', e => {
        e.preventDefault()
        let validacion = validateData(inputs, arrayVal)
        if(validacion){
            collectData(inputs, formData)
            let agregar = 'mod_repa/configuracion/perfiles/perfiles_add.php'
            let editar = 'mod_repa/configuracion/perfiles/perfiles_edit.php'
    ...
    btnEliminaPerfiles.addEventListener('click', e => {
        e.preventDefault()
        let xhr2 = new XMLHttpRequest
        let url2 = 'mod_repa/configuracion/perfiles/perfiles_use.php?id='+id
        xhr2.open('GET', url2)
        xhr2.send()
                ...
                if(respuesta2 == ''){
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            let url = 'mod_repa/configuracion/perfiles/perfiles_delete.php'
    ...
    btnCancelaPerfiles.addEventListener('click', e => {
        e.preventDefault()
        cleanInputs(inputs)
        edit = false
        activoPerfiles.checked = true
